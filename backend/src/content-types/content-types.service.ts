...
export class ContentTypesService {
  ...
  async createContentType(tenantId: string, createDto: CreateContentTypeDto) {
    ...
    return this.tenantPrisma.withTenant(tenant.db_name, async (client) => {
      ...
      if (existing.length > 0) {
        throw new BadRequestException(`Content type with collection "${createDto.collection}" already exists`);
      }
  ...
  async updateContentType(
    ...
    return this.tenantPrisma.withTenant(tenant.db_name, async (client) => {
      ...
      if (existing[0].is_system === 1) {
        throw new BadRequestException('Cannot modify system content types');
      }
      ...
      if (updateDto.collection !== undefined) {
        ...
        if (duplicate.length > 0) {
          throw new BadRequestException(`Content type with collection "${updateDto.collection}" already exists`);
        }
  ...
  async deleteContentType(tenantId: string, contentTypeId: string) {
    ...
    return this.tenantPrisma.withTenant(tenant.db_name, async (client) => {
      ...
      if (existing[0].is_system === 1) {
        throw new BadRequestException('Cannot delete system content types');
      }
      ...
      if (entries.length > 0) {
        throw new BadRequestException('Cannot delete content type with existing entries. Please delete all entries first.');
      }
  ...
  /**
   * Create a field for a content type
   */
  async createField(
    tenantId: string,
    contentTypeId: string,
