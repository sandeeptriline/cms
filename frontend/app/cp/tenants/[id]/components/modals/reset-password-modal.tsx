...
export function ResetPasswordModal({
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    ...
    setError(null)
    try {
      await tenantUsersApi.update(tenantId, userId, { password: data.password })
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Reset the password for <strong>{userEmail}</strong>. The user will need to use the new password to log in.
          </DialogDescription>
        </DialogHeader>
        {error && (
